...
def space_timesteps(num_timesteps, section_counts):
    ...
    if isinstance(section_counts, str):
        if section_counts.startswith("ddim"):
            ...
            raise ValueError(
                f"No se pueden crear exactamente {num_timesteps} pasos con un paso entero"
            )
    ...
    for i, section_count in enumerate(section_counts):
        ...
        if size < section_count:
            raise ValueError(
                f"No se puede dividir una sección de {size} pasos en {section_count}"
            )
        if section_count <= 1:
            frac_stride = 1
        else:
...
class SpacedDiffusion(GaussianDiffusion):
    ...
    def __init__(self, use_timesteps, **kwargs):
        ...
        based_diffusion = GaussianDiffusion(**kwargs)
        last_alpha_cumprod = 1.0
        new_betas = []
        for i, alpha_cumprod in enumerate(based_diffusion.alphas_cumprod):
